...
def detect_vowel(stream):
    ...
    print(f"Frecuencia dominante: {peak_freq} Hz")

    # Rangos aproximados de formantes para vocales
    if 250 <= peak_freq <= 350:  # Aproximadamente para la vocal "i"
        return True, peak_freq
    elif 400 <= peak_freq <= 500:  # Aproximadamente para la vocal "e" u "o"
        return True, peak_freq
    elif 600 <= peak_freq <= 900:  # Aproximadamente para la vocal "a"
        return True, peak_freq
    elif 200 <= peak_freq <= 300 and 500 <= peak_freq <= 800:  # Aproximadamente para la vocal "u"
        return True, peak_freq
    else:
        return False, 0
...
def get_user_input():
    ...
    input_boxes = [pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 - 100, 400, 50),
                   pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2, 200, 50),
                   pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 100, 400, 50)]
    
    input_text = ["", "", ""]
    input_titles = ["Nombre:", "RUT:", "Patología (si/no):"]
...
wb = Workbook()
ws = wb.active
ws.append(["Nombre", "RUT", "Patología", "Frecuencia"])
...
while running:
    ...
    if game_active:
        vowel_detected, freq = detect_vowel(stream)
        vowel_history.append(vowel_detected)
        vowel_history.pop(0)
        energy_history.append(freq if vowel_detected else 0)
...
for freq in energy_history:
    if freq != 0:  # Solo guardamos las frecuencias mayores que 0
        if not user_data_saved:
            # Guardamos los datos del usuario solo una vez
            ws.append([user_name, user_rut, user_pathology, freq])
            user_data_saved = True  # Evita que se guarde de nuevo
        else:
            # Solo se guarda la frecuencia (energía)
            ws.append(["", "", "", freq])

# Guardar el archivo con el nombre del usuario
wb.save(f"{user_name}_completo.xlsx")
